...
var socket = new WebSocket("wss://waffles-game-theory.herokuapp.com/ws");
...
var game = new Vue({
    ...
    methods: {
        submit: function(){
          ...
          var message = JSON.stringify({type: "submission", data: this.choice, id: id})
          socket.send(message);
        },
        message: function () {
          if (this.text != "") {
            var message = JSON.stringify({type: "message", data: this.text, id: id})
            socket.send(message)
            this.text = "";
          }
        },
